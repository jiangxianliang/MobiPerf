...
public class Checkin {
	...
	public List<MeasurementTask> checkin(ResourceCapManager resourceCapManager)
			throws IOException {
		...
		try {
			...
			resourceCapManager
					.updateDataUsage(ResourceCapManager.PHONEUTILCOST);
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				if (json != null
						&& MeasurementTask.getMeasurementTypes().contains(
								json.get("type"))) {
					try {
	 ...
	 * 
	 * @param resultArray
	 *            a JSON array of results to date
	 * @param resourceCapManager
	 *            used to update data consumption based on traffic from the
	 *            checkin
	 * @throws IOException
	 */
	public void uploadMeasurementResult(JSONArray resultArray,
			ResourceCapManager resourceCapManager) throws IOException {
	 ...
	 * more complicated way than other measurement tasks. TODO do this on
	 * checkin
	 * 
	 * @param data
	 *            Contains data to upload
	 * @throws IOException
	 */
	public void uploadRrcInferenceData(RRCTask.RRCTestData data)
			throws IOException {
		DeviceInfo info = phoneUtils.getDeviceInfo();
		...
		try {
			for (String parameter : parameters) {
				...
				String response = serviceRequest("rrc/uploadRRCInference",
						parameter);
	...
	/**
	 * Impact of packet sizes on rrc inference results. TODO do this on checkin
	 * 
	 * @param sizeData
	 *            Contains data to upload
	 */
	public void uploadRrcInferenceSizeData(RRCTask.RRCTestData sizeData) {
		DeviceInfo info = phoneUtils.getDeviceInfo();
		String network_id = phoneUtils.getNetwork();
		String[] sizeParameters = sizeData.sizeDataToJSON(network_id,
				info.deviceId);

		try {
			for (String parameter : sizeParameters) {
				Logger.w("Uploading RRC size data: " + parameter);
				String response = serviceRequest("rrc/uploadRRCInferenceSizes",
						parameter);
				Logger.w("Response from GAE: " + response);
			}
		} catch (IOException e) {
	...
	private String serviceRequest(String url, String jsonString)
		...
		String fullurl = (accountSelector.isAnonymous() ? phoneUtils
				.getAnonymousServerUrl() : phoneUtils.getServerUrl())
				+ "/"
				+ url;
